...
\subsection{Il principio di inversione della tipizzazione}
Nelle dimostrazioni induttive che seguiranno, si farà un uso sistematico del \emph{Principio di Inversione} delle regole di tipizzazione. Poiché il sistema di tipi $\lambda^{\to}_{\times, +, \mathbf{1}, \mathbf{0}}$ 
è guidato dalla sintassi (\emph{syntax-directed}), esiste una e una sola regola di tipizzazione applicabile per ogni costrutto sintattico. Il principio di inversione permette di leggere le regole formali dal basso 
verso l'alto, se un termine composto possiede validamente un tipo $\tau$, le condizioni di tipizzazione dei suoi sotto-termini immediati devono essere necessariamente soddisfatte. Ad esempio, affermare che $\Gamma 
\vdash \mathsf{fst} \, e : \tau$ significa, per inversione, che deve esistere un tipo $\sigma$ tale per cui vale necessariamente il giudizio interno $\Gamma \vdash e : \tau \times \sigma$.
