...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{CoC}                     & \checkmark & \checkmark & \checkmark & \checkmark \\ \hline
    \end{tabular}
...
\subsection{Definizione di sistema logico} 
Un sistema logico è formalmente definito dalla terna $\mathcal{L} = \langle \mathcal{S}, \mathcal{V}, \models \rangle$, dove:
\begin{itemize}
...
% TODO: aggiungere valida, sodisfacibile, contraddizione

\subsection{Classificazione dei sistemi logici}
I sistemi logici per loro natura sono moltissimi. In un certo senso, descrivono e catalogano ogni possibile verita mediante una struttura. Tale struttura consegue da qualche assioma,
che per definizione sono verita atomiche. La scelta di tali assiomi è un gesto fondamentale perche determina quali entita siano ammesse nel discorso logico. La classificazione di tali 
sistemi non è dunque un mero esercizio tassonomico, ma risulta fondamentale per comprendere l'ambito di applicazione, le potenzialità espressive e i limiti intrinseci di ogni linguaggio 
formale.

\paragraph{Metodologia classificativa.} 
Storicamente la metodologia utilizzata per classificare i sistemi logici muove dalle modifiche effettuate all'apparato della logica classica, quest'ultima rappresenta il nucleo fondativo 
che ha permesso di definire e dimostrare la stragrande maggioranza dei teoremi della matematica moderna. Riportato di seguito le direzioni principali su cui si articola la classificazione.
\begin{itemize}
    \item \textbf{L'estensione del linguaggio:} 
        consiste nell'arricchimento della sintassi classica mediante l'introduzione di nuovi operatori logici (come quelli modali o temporali), 
        ampliando il potere espressivo del sistema senza rinnegarne i principi base.
    \item \textbf{La divergenza dai principi cardine:} 
        si attua attraverso la revisione o il rifiuto di assiomi universali della logica classica (come il terzo escluso), 
        dando vita a sistemi "rivali" come la logica intuizionista o quella fuzzy.
    \item \textbf{L'analisi sottostrutturale:} 
        si focalizza sulla modifica delle regole di gestione delle premesse, trattate non più come verità astratte e inesauribili, 
        ma come risorse finite e soggette a consumo, come avviene nella logica lineare.
\end{itemize}

% TODO: Classificazione dei sistemi logici, secondo la distanza dal classico
...
sul piano semantico, quali enunciati siano conseguenza logica di altri ($\Gamma \models A$), un sistema di prova definisce un insieme finiti di regole sintattiche atte 
a derivare tali conclusioni attraverso passaggi meccanici. In tal senso, il sistema di priva rappresenta il corispettivo sintattivo della relazione di conseguenza logica
...
% TODO: Classificazione di sistemi di deduzione 
    % Sistemi assiomatici (sistemi alla Hilbert)
    % Deduzione naturale (Gentzen)
    % Calcolo dei sequenti (Gentzen)
    % Metodo dei tableaux (o alberi di refutazione)
