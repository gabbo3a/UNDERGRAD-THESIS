...
definisce la verità di una formula in termini di ciò che costituisce una sua dimostrazione, utiizza la ricorsione permessa dalla composizionalita della sintassi.
...
\begin{equation}
    \begin{array}{lrll}
        \text{Formule:} & \phi, \psi & ::= p \quad                  & \text{(Variabili proposizionali)} \\
...
\begin{equation}
    \begin{array}{lrll}
        \text{Termini:}  & t, u      & ::= x \mid c \mid f(t_1, \dots, t_n) & \text{(Variabili, Costanti, Funzioni)} \\
        \text{Formule:} & \Phi, \Psi & ::= P(t_1, \dots, t_n) \quad         & \text{(Predicati atomici)}             \\
...
\begin{equation}
    \begin{array}{lrll}
        \text{Formule:} & \phi, \psi & ::= X \quad                & \text{(Variabili proposizionali/predicative)} \\
                        &            & \mid \bot \mid \dots \quad & \text{(Connettivi IPC)}                       \\
...
\begin{equation}
    \begin{array}{lrll}
        \text{Tipi:}    & \sigma, \tau & ::= \iota \mid o \mid \sigma \to \tau                  & \text{(Individui, Formule, Funzioni)}  \\
        \text{Termini:} & M, N         & ::= x^\sigma \mid c^\sigma \quad                       & \text{(Variabili e costanti tipate)}   \\
                        &              & \mid (M N) \mid (\lambda x^\sigma. M)                  & \text{(Applicazione e Astrazione)}     \\
        \text{Formule:} & \Phi, \Psi   & ::= M^o \mid \bot \mid \dots \quad                     & \text{(Termini di tipo } o \text{)}    \\
...
\subsubsection{Combinazioni del cubo logico}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        \hline
        \textbf{Sistema Logico} & \textbf{IPC (Base)} & \textbf{Individui ($x$)} & \textbf{Ordini ($y$)} & \textbf{Tipi ($z$)} \\ 
                                & (Connettivi)        & (Primo Ordine)           & (Secondo Ordine)      & (Tipi Semplici)     \\ \hline
        IPC                     & \checkmark          & --                       & --                    & --                  \\
        IQC / IFOL              & \checkmark          & \checkmark               & --                    & --                  \\
...
\subsection{Unificazione dei costruttori in IHOL}
