...
\begin{description}[leftmargin=1.5em, style=nextline, font=\bfseries]
    \item[$\phi \land \psi$] Una prova è costituita da una coppia ordinata $\langle e_1, e_2 \rangle$ che contiene simultaneamente un'evidenza $e_1$ per $\phi$ e un'evidenza $e_2$ per $\psi$.
    \item[$\phi \lor \psi$] Una prova è una coppia $\langle i, e \rangle$ dove l'indice $i \in \{0, 1\}$ agisce da selettore: se $i=0$ l'evidenza $e$ prova il disgiunto $\phi$, mentre se $i=1$ essa prova il disgiunto $\psi$.
    \item[$\phi \to \psi$] Una prova è una costruzione funzionale $f$ in grado di trasformare ogni possibile prova di $\phi$ in una prova di $\psi$, definendo così un metodo effettivo di derivazione.
    \item[$\bot$] Rappresenta l'assurdo logico e non possiede alcuna prova; coerentemente, non è possibile fornire alcuna costruzione valida per questo enunciato.
    \item[$\neg \phi$] Equivalente a $\phi \to \bot$, la sua prova è una costruzione che trasforma ogni ipotetica evidenza di $\phi$ in una prova della contraddizione ($\bot$), mostrandone l'asseribilità impossibile.
\end{description}
...
\paragraph{Semantica.}
\begin{description}[leftmargin=1.5em, style=nextline, font=\bfseries]
    \item[Termini $M^o$] Il significato di una formula è determinato dal contenuto informativo (valore) della costruzione definita dal termine di tipo $o$; la verità risiede nell'effettiva esecuzione del processo logico descritto.
    \item[$\forall x^\sigma \Phi$] Una prova è una costruzione funzionale il cui dominio è l'intero insieme di tutti i termini (costruzioni) appartenenti al tipo $\sigma$ all'interno della gerarchia definita.
    \item[Identità] La relazione di identità tra prove è determinata dalla convertibilità (riduzione) tra termini: due prove sono considerate identiche se i processi di calcolo che esse sottendono convergono allo stesso risultato.
\end{description}
...
\begin{description}[leftmargin=3.5em, style=nextline, font=\bfseries]
    \item[Termini $M^o$] Il contenuto informativo (o valore) della costruzione definita dal termine di tipo $o$. La verità di una formula è legata alla capacità di esibire un termine che la abiti.
    \item[$\forall x^\sigma \Phi$] Una costruzione funzionale il cui dominio è l'insieme di tutti i termini appartenenti al tipo $\sigma$ nella gerarchia.
    \item[Identità] La relazione di convertibilità (riduzione) tra termini, intesa come equivalenza tra processi di calcolo che convergono allo stesso risultato.
\end{description}
